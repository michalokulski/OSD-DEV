    ...
    try {
      if ($inSwLoaded) {
        ...
        foreach ($k in @('Classes\AppID','Classes\CLSID','Classes\Interface','Classes\Typelib',
          ...
          reg Copy-Item "HKLM\ADK_IN_SW\$k" "HKLM\ADK_PE_SW\$k" /s /f 2>&1 | Out-Null
        }
      ...
      if ($inSysLoaded) {
        ...
        foreach ($k in @('ControlSet001\Services\Appinfo',
          ...
          reg Copy-Item "HKLM\ADK_IN_SYS\$k" "HKLM\ADK_PE_SYS\$k" /s /f 2>&1 | Out-Null
        }
        # TermService — copy and disable (Start=4)
        reg Copy-Item "HKLM\ADK_IN_SYS\ControlSet001\Services\TermService" `
...
function Create-StartupScript {
  ...
  # 3. WiFi setup (if enabled) — Initialize-OSDCloudStartnet -WirelessConnect
  # 4. Environment variables for Java, 7-Zip, Sysinternals, etc.
...
'@
  ...
  if ($wxRuntime) {
    # Use empty title with variable to avoid START command ambiguity with quoted paths
...
"@
    ...
    # Make shell fallback a hard error (cannot boot without shell)
